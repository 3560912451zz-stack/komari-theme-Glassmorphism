...
- 每个当前可见节点都独立显示一面旗帜，按节点 UUID 区分；相同坐标会自动微调，避免标记重叠
- 删除或隐藏节点后，标记会从地球和铺平地图立即消失；迟到的地理请求不会把旧节点写回来
- 国旗只依据节点当前的 `region` 生成；地理服务只用于城市和坐标，无法识别时使用中性 `UN` 旗帜
- IP 地理结果只保留在当前页面会话中，不写入 `localStorage`，新节点不会受到 30 天旧缓存影响
- 三种地球渲染器统一缩小旗帜尺寸，节点较多时仍能清楚区分而不遮挡地球
- 触发新的 Release 构建，确保通过仓库地址导入时拿到包含上述修复的主题包
...
<details open>
<summary><strong>v3.3.7 · 节点旗帜显示优化</strong></summary>

- 地球和铺平地图只渲染当前节点，删除或隐藏节点后立即移除对应标记
- 移除 IP 地理信息的 30 天 `localStorage` 缓存，避免旧节点或旧国家旗帜残留
- 为每个节点保留独立旗帜和稳定位置，新增节点可在当前页面重新解析
- 缩小 Cobe、Three.js 地球和铺平地图上的旗帜，减少多节点重叠

</details>

<details>
<summary><strong>v3.3.6 · 当前节点地理标记修复</strong></summary>
