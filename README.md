...
- 城市译名按“国家代码 + 城市原文”写入 Komari 的主题设置，管理员首次遇到新城市时自动翻译并保存
- 普通访客启动时直接读取服务器缓存，不再为已经处理过的城市重复请求翻译服务
- 写入前会合并最新主题设置，写入后会重新读取确认；缓存只包含城市名称和译名，不包含 IP、节点 UUID 或坐标
- 节点地理标记按当前节点实时计算，删除节点不会因为城市缓存留下旧旗帜；同一地点的中英文城市名会统一聚合

---

## 📍 v3.3.13 Null Island 地理结果修复

- 将地理服务常用的 `0,0` 未知坐标视为无效结果
- 管理员可见的 Anycast / 代理 IP 不再把南极等节点旗帜放到赤道附近，改用配置国家的代表坐标

---

## 📍 v3.3.12 管理员地球国旗定位修复

- 管理员看到的隐藏节点不再把私有 / 保留 IP 送去地理定位服务，避免出口位置污染节点坐标
- 节点配置的国家代码与地理服务结果必须严格一致；不一致时回退到该国家代表坐标，保证南极等旗帜不会漂到其他国家
- 地理查询使用与缓存键相同的公网 IP，删除或切换节点后不会复用错误位置

---

## 📍 v3.3.11 运行时城市本地化与地球回位修复

- 城市名称不再依赖手工维护词典；IP 地理服务优先请求中文结果，未提供中文时自动调用 HTTPS 翻译接口
- 翻译结果由列表、详情页和地球标记共享，新增任意节点会在当前会话自动处理，不需要再次修改主题代码
- v3.3.11 的会话缓存已在 v3.3.14 升级为服务器主题设置缓存；不写入 `localStorage`，删除节点后不会留下旧地点标记
- 翻译服务不可用时保留原始城市名，不阻塞节点地理坐标和页面加载
- `Middle` 等容易被直译成普通词的地名可保留原名，马恩岛节点不再显示成“中间”
- Tab / Esc 退出全屏地球时复用同一套渲染画布，只缩放并移动完整地球，消除回位末帧的白闪和右下抽动

---

## 📍 v3.3.10 地球旗帜与地理解析修复

- 地球旗帜与首页节点卡片统一使用节点当前 `region`；IP 地理服务只在国家一致时提供城市坐标，避免旗帜漂到别的国家
- 地理解析状态按节点 UUID、当前 IP 和地区隔离；节点删除、换 IP 或换地区后不会复用旧位置
- 未命中的城市词条保留服务商返回的原名，详情页和节点列表不再静默隐藏城市
- 不恢复本地持久化的 30 天 IP 地理缓存，当前页面之外不会保存解析结果

---

## 📍 v3.3.9 详情页地理位置

- 详情页节点名称下新增紧凑的位置行，显示城市与国家 / 地区
- 城市优先使用当前会话中的地理解析结果，未解析时先显示节点公开的国家 / 地区
- 延续现有权限控制，普通访客不会看到节点 IP 或受保护的精确地理信息

---

## 🌍 v3.3.8 同地点旗帜聚合

- 同一国家、同一城市的多个当前节点只显示一面旗帜，例如多台洛杉矶机器共用一个洛杉矶标记
- 同一国家的不同城市仍分别显示；地理服务未识别出城市时按节点独立保留，不会退回“整个国家一面旗帜”
- 地点标记会记录该地点的节点数与在线数，顶部在线 / 离线统计仍按真实机器数量计算
- 继续保持无 30 天地理缓存，删除或隐藏节点后会立即重新计算当前地点标记
...
<details open>
<summary><strong>v3.3.11 · 运行时城市本地化与地球回位修复</strong></summary>

- 移除手工城市中文词典，改为 IP 地理服务中文字段与 HTTPS 自动翻译回退
- 列表、详情页和地球标记共享 `cityZh`，新增节点不需要人工补城市名
- 使用当前页面缓存并限制请求去重；v3.3.14 起新增管理员维护的城市译名服务器缓存，但仍不恢复 IP 地理坐标的 30 天持久缓存
- 翻译失败时保留原始城市名，不影响坐标、国旗和节点显示
- 保留容易被通用翻译器误译的地点专名，当前先覆盖马恩岛 `Middle`
- 全屏地球退出后保留全屏 Canvas backing buffer，并通过父层 FLIP 变换回到行内位置
- 新增逐帧回归，检查 Canvas、宿主和国旗在 Tab / Esc 回挂边界保持连续

</details>

<details open>
<summary><strong>v3.3.8 · 同地点旗帜聚合</strong></summary>

- 同一国家、同一城市的多个节点合并为一个地球或地图标记
- 同国不同城市继续分开，城市未知的节点保持独立，避免重新按国家粗略合并
- 地点标记累计节点数与在线数，首页在线 / 离线总数仍按机器统计
- 保留当前会话地理解析与节点删除后的即时裁剪，不恢复 30 天持久缓存
