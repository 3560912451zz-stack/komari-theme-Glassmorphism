...
## 2026-08-10 Tab immersive earth (M4/M5/M6)

- Home now treats a plain, unmodified `Tab` on non-interactive page content as an immersive-earth toggle; a second `Tab` or `Escape` restores the inline globe. Native keyboard navigation remains unchanged for inputs, links, buttons, editable controls, active dialogs, modifier chords and `Shift+Tab`.
- Before entry, `HomeView` measures summary and node wrappers in or within 64 px of the viewport, sorts them by visual row/column position, and assigns exact left/right viewport exit vectors. Summary and node cards use separate sequences; visible nodes start at 20 ms with a 25 ms stagger (20-195 ms for 8 cards) and a 400 ms duration, so the last card finishes at about 595 ms with the 600 ms Earth motion. Return uses a 15 ms reverse stagger, while an interrupted transition drops all delay so rapid repeated `Tab` presses continue from the current visual state.
- `NodeGeneralCards` keeps one renderer instance alive and moves it through Vue `Teleport`. A fixed fullscreen stage provides viewport placement and exit controls, while a dedicated `earth-motion-frame` performs a WAAPI FLIP with GPU `transform`; the renderer changes to its final layout once instead of being resized on every animation frame.
- The globe target is centered and bounded by `min(86vw, 68dvh, 36rem)` (about 490 px at 1280x720), rather than filling the viewport. Tiled mode stays aspect-preserving within 88 vw / 68 dvh / 1024 px. The fullscreen stage stays completely transparent in both themes, preserving the configured page background instead of washing it out.
- The viewport stage locks document scrolling without changing `scrollbar-gutter`, provides an icon-only minimize action, retains the original header layout through a placeholder, exits immediately on KeepAlive deactivation/settings changes, and respects both `disablePageAnimation` and `prefers-reduced-motion`. Realistic/Cobe offsets live on the responsive placeholder so returning after a viewport breakpoint change cannot jump.
- The detached Earth remains at `z-index: 70`; only during `entering` and `returning`, `HomeView` is temporarily raised to `z-index: 71` with pointer events disabled. Cards therefore cross in front of the Earth during both directions, while the stable immersive Earth remains interactive after the page layer returns to its normal stacking level.
- The home node toolbar now exits as two coherent segments instead of fading in place: group tabs and quick filters move left, while view/search actions move right. Entry uses 0/25 ms delays, return reverses at 0/15 ms, both run for 400 ms, and `prepareEarthSceneMotion()` calculates the exact X distance needed to clear the current viewport by 32 px so the short action segment does not streak across a full viewport.
- The slowed scene uses `cubic-bezier(0.3, 0.7, 0.3, 1)` across Earth FLIP, stage geometry, summary cards, node cards and toolbar segments. This replaces the heavily front-loaded ease-out so the visible movement uses the full 600/400 ms durations instead of completing most travel in the first frame window; unrelated page-entry and hover transitions retain their original timing.
- Playwright fixture settings now allow animation to be enabled per test. Behavioral coverage checks `Tab`/`Escape`, centered compact geometry, ordered card exits beyond the viewport, restored card/Earth geometry, scroll locking, native search-input focus navigation, stable motion-frame layout, tiled aspect preservation, both rapid reversal directions, disabled animation and reduced motion.
- Focused runtime verification with local Edge passed 7/7 tests covering normal entry/exit, ordered card motion, FLIP stability, tiled geometry, both rapid reversals and disabled animation. A four-frame 1280x720 capture confirmed the Earth frame moves from 448 px inline to about 490 px centered and server cards remain visible while leaving in order. Headless Edge did not render realistic WebGL pixels, so the captures validate layout and surrounding motion rather than globe canvas content.
- Final static verification passed read-only ESLint, application and Node TypeScript checks, `bunx playwright test --list` (19 tests), `git diff --check`, and `bun run build-only` (1852 modules). The transparent-stage follow-up passed its focused local Edge test 1/1. The layer/timeline and reduced-motion focused Edge cases pass 2/2; the latter also fixed a scoped-CSS specificity bug that had allowed the 320 ms node transition to override `prefers-reduced-motion`.
- Residual browser coverage is intentionally narrow: immersive motion has been exercised at 1280x720 in card mode. List-row immersive exit and mobile immersive geometry are implemented by the same selectors but have not yet been run as behavioral browser cases.

## 2026-08-10 local node-card motion preview
...
## 2026-08-16 preserve Middle proper name (M6)

- Added a runtime translation bypass for the exact city key `Middle`, so the Isle of Man place name remains `Middle` instead of being translated literally to `中间`.
- The bypass is shared by IP geo results, node list metadata, detail location text, and earth cluster labels; no network translation request is made for this key.
- Validation passed: `bun run type-check`, targeted ESLint, a no-fetch helper smoke check, `bun run build-only --configLoader runner`, and `git diff --check`.

## 2026-08-16 immersive earth renderer continuity fix (M4/M6)

- The previous two exit fixes were incomplete. They stabilized the outer frame, but then visibly resized the realistic renderer from its fullscreen dimensions to its inline dimensions. Direct frame sampling caught the bad paint: the Canvas grew from 474.88 px to 518.97 px while its host stayed at 474.88 px, then returned one frame later as the backing buffer changed from 734 px to 672 px. Fixed-pixel HTML flags changed size at the same boundary. The old DPR-ratio readiness check could not detect this because both renderer sizes used approximately the same 1.5 ratio.
- The realistic renderer is now initialized at its fullscreen size while hidden, synchronously sizing the Three.js renderer, post-processing composer, perspective camera, scene container and HTML overlay layers before it may pause. This also prevents a newly loaded `globe.gl` Canvas from exposing its default viewport-sized 1280x720 buffer when Tab is pressed immediately after page load.
- Once fullscreen sizing is ready, `NodeGeneralCards` pins that physical frame and backing buffer. Exit uses only the parent WAAPI transform to scale and translate the complete renderer tree to the inline placeholder, keeps the terminal transform through Teleport, and deliberately retains the pinned renderer while inline. A later entry reuses the same buffer instead of resizing it again.
- Inline resize handling recomputes only the pinned parent transform. Forced teardown/settings paths still clear the pin while hidden. Dynamic `scrollbar-gutter` changes were removed, and the post-unlock endpoint is resynchronized to the live placeholder before paint.
- The exit regression now samples every animation frame for both Tab and Escape. It requires an unchanged fullscreen backing buffer, Canvas/host agreement on every paint, continuous frame/Canvas/flag geometry across Teleport, a visible inline renderer and a final frame aligned to the unlocked placeholder. This test catches the exact intermediate frame missed by the earlier endpoint-only checks.
- Validation passed: `bun run type-check`, targeted ESLint, `bun run build-only --configLoader runner`, `git diff --check`, and all 10 production-preview immersive interaction tests. In-app browser verification at 1280x720 measured a 489.59 px fullscreen frame with a 734x734 backing buffer; after exit the visual frame returned to 448 px with the same 489.59 px physical Canvas and 734x734 buffer, remaining visible and aligned to the placeholder within about 0.005 px.
